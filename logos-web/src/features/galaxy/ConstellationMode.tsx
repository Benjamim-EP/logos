...
import type { Note } from "@/types/galaxy" // Importamos o tipo original

// Criamos um tipo local que estende a nota original com a distância calculada
type NoteWithDistance = Note & { distance?: number };

export function ConstellationMode() {
  // CORREÇÃO 1: Trocamos 'notes' por 'allNotes' (nome correto no store)
  const { focusNode, setFocusNode, allNotes } = useGalaxyStore()
  const { setSelectedNote } = useSelectionStore()
  const [isScanning, setIsScanning] = useState(true)

  // 1. Calcula vizinhos (Top 15 mais próximos)
  // CORREÇÃO 2: Castamos o retorno para NoteWithDistance[]
  const neighbors = useMemo(() => {
    if (!focusNode) return []
    return getNearestNotes(focusNode, allNotes, 15) as NoteWithDistance[]
  }, [focusNode, allNotes])
  ...
  const getRelativePos = (noteX: number, noteY: number) => {
    const screenX = window.innerWidth / 2 + (noteX - focusNode.x) * 1.5
      ...
      <div className="relative w-full h-full">
        ...
        >
          <div className="absolute -bottom-8 text-center w-60">
            <p className="text-white font-bold text-lg">{focusNode.title}</p>
          </div>
        </motion.div>
          ...
          return (
            ...
            >
              <div className="absolute top-8 left-1/2 -translate-x-1/2 w-40 text-center opacity-70 hover:opacity-100">
                ...
                {/* Agora o TS entende que 'distance' existe aqui */}
