...
import { Loader2, Mail, ArrowRight, Compass } from "lucide-react"
      ...
      <div className="w-full lg:w-1/2 flex items-center justify-center p-6 relative z-20">
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }} className="w-full max-w-md space-y-8">
          ...
          <div className="text-center space-y-2">
            <h2 className="text-3xl font-bold tracking-tight text-white">{t('auth.welcome_back')}</h2>
            <p className="text-sm text-gray-400">{t('auth.access_galaxy')}</p>
          </div>

          <div className="space-y-4">
            <Button onClick={handleGoogleLogin} disabled={!!isLoading} className="w-full h-12 bg-white text-black hover:bg-gray-200 font-semibold text-base flex items-center justify-center gap-3">
            ...
            <div className="relative">
              ...
              <div className="relative flex justify-center text-xs uppercase"><span className="bg-[#050505] px-2 text-gray-500">OU</span></div>
            </div>

            <Button onClick={handleStandardLogin} disabled={!!isLoading} variant="outline" className="w-full h-12 border-white/10 bg-white/5 hover:bg-white/10 text-gray-300 justify-between group">
              <span className="flex items-center gap-2"><Mail className="w-4 h-4" /> {t('auth.login_email')}</span>
              {isLoading === 'standard' ? <Loader2 className="w-4 h-4 animate-spin" /> : <ArrowRight className="w-4 h-4 opacity-50 group-hover:translate-x-1" />}
            </Button>

            <Button onClick={handleTourClick} variant="ghost" className="w-full h-10 text-cyan-400 hover:text-cyan-300 hover:bg-cyan-950/30 font-medium text-sm border border-transparent hover:border-cyan-500/20">
               <Compass className="w-4 h-4 mr-2" /> {t('auth.tour_mode')}
            </Button>
          </div>

          <div className="text-center text-sm text-gray-500 mt-6">
            Não tem uma conta? <button onClick={handleStandardLogin} className="text-blue-400 hover:underline">{t('auth.register')}</button>
          </div>
